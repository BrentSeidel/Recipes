...
For the purposes of explanation, assume that the strands are all laid out horizontally and numbered from the top to the bottom such that the top strand is number 1.  The notation ``1-over-2'' would mean to move the top strand over the second from the top strand.  ``1-under-2'' would mean to move the top strand under the second strand.  Moves can be more complicated as ``7-under-6-over-5'' would move strand 7 under strand 6 and then over strand 5.  Each move starts with the strands being numbered with 1 at the top.  Note that to be a valid, a move has to describe what happens which each strand affected during a move.  Thus ``4-over-2'' is not a valid move because it doesn't say what strand 4 should do with strand 3.
...
\begin{itemize}
  ...
  \item Move the top strand over the lower strand (1-over-2).
...
\begin{itemize}
  ...
  \item Roll each lump into long strand.
  \item Lay the strands side-by-side and squeeze one end together.
  \item Move the top strand over the middle strand (1-over-2), then move the bottom strand over the middle strand (3-over-2).
  \item Repeat until the end of the strands is reached.
  \item Squeeze the last end together.
\end{itemize}
...
\begin{itemize}
  ...
  \item Roll each lump into long strand.
  \item Lay the strands side-by-side and squeeze one end together.
  \item Move the top strand over the adjacent strand, and under the next strand (1-over-2-under-3).
  \item Repeat until the end of the strands is reached.
  \item Squeeze the last end together.
\end{itemize}
